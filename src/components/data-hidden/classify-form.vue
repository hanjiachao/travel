          ...
          <Row :gutter="20">
            <Col v-for="(formItem, fromIndex) in card.formItems" :span="formItem.span? formItem.span : formItem.type === 'card' ? 24 : 8" :key="fromIndex">
                      ...
                      <Row :gutter="20">
                        <Col v-for="(childrenItem, childrenIndex) in formItem.childrenForm" :key="childrenIndex" :span="childrenItem.span? childrenItem.span : 8">
                          <formItem :label="childrenItem.label">
                            ...
                            <form-select v-else-if="childrenItem.type === 'select' && childrenItem.dictionary" v-model="childrenCard[childrenItem.value]" :config="childrenItem.config ? formItem.config : {}" :wordType="childrenItem.options" :placeholder="childrenItem.placeholder? childrenItem.placeholder : '请选择' + childrenItem.label"></form-select>
                            <Select size="large" v-model="childrenCard[childrenItem.value]" v-else-if="childrenItem.type === 'select'" :placeholder="childrenItem.placeholder? childrenItem.placeholder : '请选择' + childrenItem.label">
                              <Option v-for="(option, optionIndex) in options[childrenItem.value]" :key="optionIndex" :value="option.value">{{ option.label }}</Option>
                            </Select>
              ...
              <formItem v-else-if="formItem.showItem" :label="formItem.label" :prop="formItem.value">
                <form-select-page v-if="formItem.type === 'selectPaging'" :config="formItem.config || {}" v-model="formData[formItem.value]" :placeholder="formItem.placeholder? formItem.placeholder : '请选择' + formItem.label"></form-select-page>
                <tree-select v-else-if="formItem.type === 'treeSelect'" :config="formItem.config || {}" v-model="formData[formItem.value]" :placeholder="formItem.placeholder? formItem.placeholder : '请选择' + formItem.label"></tree-select>
                <form-cascader v-else-if="formItem.type === 'cascader'" :config="formItem.config || {}" v-model="formData[formItem.value]" :wordType="formItem.options" :placeholder="formItem.placeholder? formItem.placeholder : '请选择' + formItem.label"></form-cascader>
                <form-select :disabled="formItem.disabled" v-else-if="formItem.type === 'select' && formItem.dictionary" :config="formItem.config || {}" :wordType="formItem.options || 'temp'" v-model="formData[formItem.value]" :placeholder="formItem.placeholder? formItem.placeholder : '请选择' + formItem.label"></form-select>
                <Select size="large" v-else-if="formItem.type === 'select'" v-model="formData[formItem.value]" :placeholder="formItem.placeholder? formItem.placeholder : '请选择' + formItem.label">
                  <Option v-for="(option, optionsIndex) in options[formItem.value]" :key="optionsIndex" :value="option.value">{{ option.label }}</Option>
                </Select>
                <DatePicker :format="formItem.format || 'yyyy-MM-dd'" v-model="formData[formItem.value]" v-else-if="formItem.type === 'date'" style="width: 100%" :value="formData[formItem.value]" @on-change="(a, b) => { dateChange (a, b, formItem.value)}" :start-date="formItem.startDate ? new Date(formItem.startDate) : new Date()" size="large" type="date" :placeholder="formItem.placeholder? formItem.placeholder : '请选择' + formItem.label"></DatePicker>
                <el-upload
                  action=""
                ...
                <Input v-else size="large" :rows="3" :type="formItem.type? formItem.type : 'text'" v-model="formData[formItem.value]" :placeholder="formItem.placeholder? formItem.placeholder : '请输入' + formItem.label" />
              </formItem>
            </Col>
          </Row>
        </div>
...
import fromCascader from './form-cascader.vue'
import treeSelect from './tree-select.vue'
import formSelectPage from './form-select-paging.vue'
export default {
  name: 'classify-form',
  components: {
        ...
        let formItems = cardItem.formItems.map(formItem => {
          ...
          if (formItem.type === 'card') {
            ...
            formItem.childrenForm.map(childrenItem => {
              ...
              if (childrenItem.type === 'select') {
                if (typeof childrenItem.options === 'string') {
                  childrenItem.dictionary = true
                } else {
                  _this.$set(_this.options, childrenItem.value, childrenItem.options)
                }
              }
            })
            ...
            if (formItem.type === 'image' || formItem.type === 'file' || formItem.type === 'cascader') {
