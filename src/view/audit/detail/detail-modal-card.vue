  ...
  <div class="card-list">
    <Card class="card-item" v-for="(cardItem, cardIndex) in hideItemList" :key="cardIndex">
      ...
      <div v-show="showCase[cardItem.showCase]" class="card-body">
        <div v-if="cardItem.type === 'trans'" class="tans-info">
          <Row :gutter="20">
            <Col :span="10">
              <Card v-if="detail[cardItem.transInfo.rollOut]">
                <div class="trans-item" v-for="(transItem, tansIndex) in cardItem.formItems" :index="tansIndex">
                  <span class="label">
                    {{ transItem.label }}：
                  </span>
                  <span>
                    {{ means.getKeyValue(transItem.value, detail[cardItem.transInfo.rollOut]) }}
                  </span>
                </div>
              </Card>
            </Col>
            <Col style="text-align: center;" :span="4">
              <img class="arrow" src="../../../assets/images/trans.png">
            </Col>
            <Col :span="10">
              <Card v-if="detail[cardItem.transInfo.shiftInto]">
                <div class="trans-item" v-for="(transItem, tansIndex) in cardItem.formItems" :key="tansIndex">
                  <span class="label">
                    {{ transItem.label }}：
                  </span>
                  <span>
                    {{ means.getKeyValue(transItem.value, detail[cardItem.transInfo.shiftInto]) }}
                  </span>
                </div>
              </Card>
            </Col>
        ...
        <div v-else v-for="(infoItem, infoIndex) in cardItem.formItems" :key="infoIndex" class="item-info" :style="{ width: infoItem.allCol ? '100%' : '50%' }" :class="infoItem.inlineCard ? 'inline-card' : ''">
    ...
    <Card class="card-item" v-if="historyData">
      <div slot="title" class="card-title" @click="showRecord = !showRecord">
        审核记录
        <Icon v-if="showRecord" class="title-icon" type="ios-arrow-down" />
        <Icon v-else class="title-icon" type="ios-arrow-forward" />
      </div>
      <div v-show="showRecord" class="card-body">
        <detail-audit :history="historyData"></detail-audit>
      </div>
    </Card>
  </div>
</template>
...
import detailAudit from './detail-audit.vue'
export default {
  name: 'detail-modal-card-audit',
  props: {
    hideItem: {
      type: Object,
      default: () => {}
  ...
  computed: {
    ...
    historyData () {
      ...
      let _this = this
      let temp = JSON.parse(JSON.stringify(this.history))
      if (this.detail[temp.extraUrl]) {
        temp.url = temp.url + '/' + this.detail[temp.extraUrl]
      } else {
        temp.url = ''
      }
