<template>
  <div>
    <Select label-in-value :disabled="disabled" v-model="option" :size="configObj.size || 'large'" :clearable="configObj.clearable" @on-change="giveValue" :placeholder="placeholder">
...
export default {
  ...
  watch: {
    ...
    wordType: {
      handler () {
        if (this.wordType instanceof Array) {
          this.options = this.wordType
          return
        }
        this.options = [{
          value: this.value || '',
